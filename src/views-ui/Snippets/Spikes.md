...
let { GPUComputationRenderer } = await import('/lib/threejs-r112/examples/jsm/misc/GPUComputationRenderer.js')
...
let makeAPI = ({ renderer }) => {
  ...
  var material = new THREE.ShaderMaterial({
    ...
    uniforms: {
      mainColor: { value: new THREE.Color('#ffff00') },
      time: { value: 0 },
      opacity: { value: 0.5 },
      posTex: { value: null },
...
env.run = async (api) => {
  let item = makeAPI({ renderer: api.renderer })
  api.scene.add(item.renderable)
  api.loop(() => {
    item.compute()
  })

  item.renderable.position.z = -10;
  item.renderable.scale.x = 1.0;
  item.renderable.scale.y = 1.0;
  item.renderable.scale.z = 1.0;

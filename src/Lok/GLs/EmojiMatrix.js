...
export const makeWords = async ({ api, mounter, vm, parent, camera, scene }) => {
  ...
  function setupWord ({ priWord = '黃', secWord = '樂' }) {
    let primaryWord2D = new TextCanvas(priWord, { fontFamily: 'Arial', wordWrap: 300, textColor: '#ffffff', textAlign: 'center' }, 32)
    let secondaryWord2D = new TextCanvas(secWord, { fontFamily: 'Arial', wordWrap: 300, textColor: '#ffffff', textAlign: 'center' }, 32)
  ...
  // let word1 = window.prompt('Please enter first name.') || '✩'
  // let word2 = window.prompt('Please enter last name.') || '✩'
  let word1 = '💎'
  let word2 = '⭐️'
  let { primaryWord, secondaryWord } = setupWord({ priWord: word1, secWord: word2 })

  api.updateWords = ({ word1, word2 }) => {
    let { primaryWord, secondaryWord } = setupWord({ priWord: word1, secWord: word2 })
  ...
  api.setWidthFor = (v) => {
    if (v === 'emoji') {
      uniforms.charWidth.value = 1.2
    } else if (v === 'words') {
      uniforms.charWidth.value = 1.0
    }
  }

  // video.addEventListener('click', () => {
  //   vm.openEmoji = !vm.openEmoji
  //   this.$forceUpdate()
...
void main () {
  ...
  vec3 hsl = rgb2hsv(vidColor.rgb);

  vec4 rainbow = vec4(hsv2rgb(hsl.xyz), 1.0) * 1.68;

  rainbow.a *= avg;

  // float ravg = 1.0 - (rainbow.x + rainbow.y + rainbow.z / 3.0);

  if (avg > 0.5) {
    vec4 fColor = texture2D(primaryWord, glpc);
    // gl_FragColor = vec4((1.0 - fColor.rgb) * finalColor.rgb, finalColor.a * fColor.a);

    gl_FragColor = vec4(vec3((fColor.r + fColor.g + fColor.b) / 3.0), fColor.a) * rainbow;
  } else {
    vec4 wColor = texture2D(secondaryWord, glpc);
    // gl_FragColor = vec4((1.0 - wColor.rgb) * finalColor.rgb, finalColor.a * wColor.a);
    gl_FragColor = vec4(vec3((wColor.r + wColor.g + wColor.b) / 3.0), wColor.a)  * rainbow;
  }
